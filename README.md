...
A tiny, self-contained tool to **load your pharmacist prompt from a text file**, run a batch of **customer-style test inputs**, and **store results**.
...
- **Where is the prompt?** → [prompt.txt](./prompt.txt)
...
See [prompt.txt](./prompt.txt) (the runner uses it as the system message).